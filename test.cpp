...
TEST(tc, io_bin) {
    int* edges;
    std::pair<int, uint64_t> info_pair = read_binfile_to_arclist("test_io.bin", edges);
...
TEST(tc, io_nvgraph) {
    int* edges;
    std::pair<int, uint64_t> info_pair = read_binfile_to_arclist("test_io_nvgraph.bin", edges);
    uint64_t trcount = GpuForward(edges, info_pair.first, info_pair.second);
    EXPECT_EQ(trcount, 3); 
    free(edges);
}
...
TEST(split, io_complete_4) {
    int* edges;
    std::pair<int, uint64_t> info_pair = read_binfile_to_arclist("test_complete_4.bin", edges);
...
TEST(cpu, io_bin) {
    int* arcs;
    std::pair<int, uint64_t> info_pair = read_binfile_to_arclist("test_io.bin", arcs);
...
TEST(cpu, io_nvgraph) {
    int* arcs;
    std::pair<int, uint64_t> info_pair = read_binfile_to_arclist("test_io_nvgraph.bin", arcs);
    uint64_t tcount = CpuForward(arcs, info_pair.first, info_pair.second);
    EXPECT_EQ(tcount, 3);    
    free(arcs);
}
